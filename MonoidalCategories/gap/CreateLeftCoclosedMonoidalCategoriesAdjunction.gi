...
##
InstallGlobalFunction( CAP_INTERNAL_FUNC_FOR_LEFT_COCLOSED_MONOIDAL_STRUCTURES_ADJUNCTION,
...
##
InstallGlobalFunction( WriteFileForLeftCoclosedMonoidalStructureAdjunction,
  function( key_val_rec, package_name, files_rec )
    local dir, L, files, header, file, source, target;
    
    L := CAP_INTERNAL_FUNC_FOR_LEFT_COCLOSED_MONOIDAL_STRUCTURES_ADJUNCTION( key_val_rec, package_name );
    
    dir := Concatenation( PackageInfo( "MonoidalCategories" )[1].InstallationPath, "/gap/" );
    
    files := [ "LeftCoclosedMonoidalCategoriesAdjunction_gd",
               "LeftCoclosedMonoidalCategoriesAdjunctionMethodRecord_gi",
               "LeftCoclosedMonoidalCategoriesAdjunctionDerivedMethods_gi",
               ];
