...
##
InstallGlobalFunction( CAP_INTERNAL_FUNC_FOR_LEFT_CLOSED_MONOIDAL_STRUCTURES_ADJUNCTION,
...
##
InstallGlobalFunction( WriteFileForLeftClosedMonoidalStructureAdjunction,
  function( key_val_rec, package_name, files_rec )
    local dir, L, files, header, file, source, target;
    
    L := CAP_INTERNAL_FUNC_FOR_LEFT_CLOSED_MONOIDAL_STRUCTURES_ADJUNCTION( key_val_rec, package_name );
    
    dir := Concatenation( PackageInfo( "MonoidalCategories" )[1].InstallationPath, "/gap/" );
    
    files := [ "LeftClosedMonoidalCategoriesAdjunction_gd",
               "LeftClosedMonoidalCategoriesAdjunctionMethodRecord_gi",
               "LeftClosedMonoidalCategoriesAdjunctionDerivedMethods_gi",
               ];
