...
\begin{tikzpicture}
	\def\yfactor{0.5}
	\node [align=right, text width=1.5cm] at (-1,0*\yfactor) {\scriptsize{$\Acal\left(\Ccal(R)^\add\right)$}};
	\node [align=right, text width=1.5cm] at (-1,-1*\yfactor) {\scriptsize{$\Ccal(R)^\add$}};
	\node [align=right, text width=1.5cm] at (-1,-2*\yfactor) {\scriptsize{$\Ccal(R)$}};
	\node [align=right, text width=1.5cm] at (-1,-3*\yfactor) {\scriptsize{$R$}};
	\def\x{0}
