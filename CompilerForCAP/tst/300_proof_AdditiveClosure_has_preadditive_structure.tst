...
Proposition:
AdditiveClosure( a preadditive category ) is equipped with a preadditive struc\
ture.
...
Lemma 1:
In AdditiveClosure( a preadditive category ), the addition of morphisms define\
s a morphism:
For two objects A and B and two morphisms alpha : A → B and beta : A → B w\
e have
function ( cat, A, B, alpha, beta )
    return IsWellDefinedForMorphismsWithGivenSourceAndRange( cat, A, 
       AdditionForMorphisms( cat, alpha, beta ), B );
end
gap> PrintLemma( );
We have to show
function ( cat_1, A_1, B_1, alpha_1, beta_1 )
    if
     not IsList( List( [ 1 .. Length( ObjectList( A_1 ) ) ], function ( i_2 )
                   return List( [ 1 .. Length( ObjectList( B_1 ) ) ], 
                      function ( j_3 )
                           return 
                            AdditionForMorphisms( UnderlyingCategory( cat_1 )
                               , MorphismMatrix( alpha_1 )[i_2][j_3], 
                              MorphismMatrix( beta_1 )[i_2][j_3] );
                       end );
               end ) ) then
        return false;
...
Lemma 2:
In AdditiveClosure( a preadditive category ), addition of morphisms is associa\
tive:
For two objects A and B and three morphisms alpha : A → B, beta : A → B, a\
nd gamma : A → B we have
function ( cat, A, B, alpha, beta, gamma )
    return 
     IsCongruentForMorphisms( cat, 
       AdditionForMorphisms( cat, AdditionForMorphisms( cat, alpha, beta ), 
         gamma ), AdditionForMorphisms( cat, alpha, 
         AdditionForMorphisms( cat, beta, gamma ) ) );
end
gap> ApplyLogicTemplate(
>     rec(
...
Lemma 3:
In AdditiveClosure( a preadditive category ), addition of morphisms is commuta\
tive:
For two objects A and B and two morphisms alpha : A → B and beta : A → B w\
e have
function ( cat, A, B, alpha, beta )
    return IsCongruentForMorphisms( cat, AdditionForMorphisms( cat, alpha, 
         beta ), AdditionForMorphisms( cat, beta, alpha ) );
end
gap> ApplyLogicTemplate(
>     rec(
...
Lemma 4:
In AdditiveClosure( a preadditive category ), composition is additive in the f\
irst component:
For three objects A, B, and C and three morphisms alpha : A → B, beta : A → B,
and phi : B → C we have
function ( cat, A, B, C, alpha, beta, phi )
    return 
     IsCongruentForMorphisms( cat, 
       PreCompose( cat, AdditionForMorphisms( cat, alpha, beta ), phi ), 
       AdditionForMorphisms( cat, PreCompose( cat, alpha, phi ), 
         PreCompose( cat, beta, phi ) ) );
end
gap> ApplyLogicTemplate(
>     rec(
...
Lemma 5:
In AdditiveClosure( a preadditive category ), composition is additive in the s\
econd component:
For three objects A, B, and C and three morphisms alpha : B → C, beta : B → C,
and phi : A → B we have
function ( cat, A, B, C, alpha, beta, phi )
    return 
     IsCongruentForMorphisms( cat, 
       PreCompose( cat, phi, AdditionForMorphisms( cat, alpha, beta ) ), 
       AdditionForMorphisms( cat, PreCompose( cat, phi, alpha ), 
         PreCompose( cat, phi, beta ) ) );
end
gap> ApplyLogicTemplate(
>     rec(
>         variable_names := [ "cat", "source", "target", "list", "func1", "func2" ],
...
Lemma 6:
In AdditiveClosure( a preadditive category ), the zero morphism between two ob\
jects is a morphism:
For two objects A and B we have
function ( cat, A, B )
    return IsWellDefinedForMorphismsWithGivenSourceAndRange( cat, A, 
       ZeroMorphism( cat, A, B ), B );
end
gap> PrintLemma( );
We have to show
function ( cat_1, A_1, B_1 )
    if
     not IsList( List( [ 1 .. Length( ObjectList( A_1 ) ) ], function ( i_2 )
                   return List( [ 1 .. Length( ObjectList( B_1 ) ) ], 
                      function ( j_3 )
                           return ZeroMorphism( UnderlyingCategory( cat_1 ), 
                              ObjectList( A_1 )[i_2], ObjectList( B_1 )[j_3] )
                            ;
                       end );
               end ) ) then
        return false;
...
function ( cat, A, B, alpha )
    return 
     IsCongruentForMorphisms( cat, 
       AdditionForMorphisms( cat, ZeroMorphism( cat, A, B ), alpha ), alpha );
end
gap> ApplyLogicTemplate(
>     rec(
...
Lemma 9:
In AdditiveClosure( a preadditive category ), the additive inverse of a morphi\
sm defines a morphism:
For two objects A and B and a morphism alpha : A → B we have
function ( cat, A, B, alpha )
    return IsWellDefinedForMorphismsWithGivenSourceAndRange( cat, A, 
       AdditiveInverseForMorphisms( cat, alpha ), B );
end
gap> PrintLemma( );
We have to show
...
Lemma 10:
In AdditiveClosure( a preadditive category ), additive inverses are left inver\
se:
For two objects A and B and a morphism alpha : A → B we have
function ( cat, A, B, alpha )
    return 
     IsCongruentForMorphisms( cat, 
       AdditionForMorphisms( cat, AdditiveInverseForMorphisms( cat, alpha ), 
         alpha ), ZeroMorphism( cat, A, B ) );
end
gap> ApplyLogicTemplate(
>     rec(
...
Lemma 11:
In AdditiveClosure( a preadditive category ), additive inverses are right inve\
rse:
For two objects A and B and a morphism alpha : A → B we have
function ( cat, A, B, alpha )
    return IsCongruentForMorphisms( cat, AdditionForMorphisms( cat, alpha, 
         AdditiveInverseForMorphisms( cat, alpha ) ), 
       ZeroMorphism( cat, A, B ) );
end
gap> ApplyLogicTemplate(
>     rec(
>         variable_names := [ "cat", "alpha" ],
