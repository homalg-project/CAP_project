...
function ( cat, arg2, arg3 )
    return List( [ 1 .. RankOfObject( arg3 ) * RankOfObject( arg2 ) ], function ( logic_new_func_30513_x )
            return ObjectifyWithAttributes( rec(
                     ...
                     ), ObjectType( cat ), CapCategory, cat, RankOfObject, RankOfObject( arg3 ) ), UnderlyingMatrix, ConvertRowToMatrix( CertainRows( HomalgIdentityMatrix( RankOfObject( arg3 ) * RankOfObject( arg2 ), UnderlyingRing( cat ) ), [ logic_new_func_30513_x ] ), RankOfObject( arg3 ), RankOfObject( arg2 ) ) );
        end );
end
########
...
function ( cat, arg2 )
    return ObjectifyWithAttributes( rec(
           ), ObjectType( cat ), CapCategory, cat, RankOfObject, Sum( List( arg2, function ( logic_new_func_30873_x )
                return RankOfObject( logic_new_func_30873_x );
            end ) ) );
end
########
...
function ( cat, objects, T, tau, P )
    return ObjectifyWithAttributes( rec(
           ), MorphismType( cat ), CapCategory, cat, Source, P, Range, T, UnderlyingMatrix, UnionOfColumns( UnderlyingRing( cat ), RankOfObject( T ), List( tau, function ( logic_new_func_31855_x )
                return UnderlyingMatrix( logic_new_func_31855_x );
            end ) ) );
end
########
...
function ( cat, objects, T, tau, P )
    return ObjectifyWithAttributes( rec(
           ), MorphismType( cat ), CapCategory, cat, Source, T, Range, P, UnderlyingMatrix, UnionOfRows( UnderlyingRing( cat ), RankOfObject( T ), List( tau, function ( logic_new_func_32026_x )
                return UnderlyingMatrix( logic_new_func_32026_x );
            end ) ) );
end
########
